...
namespace QuanLyPhongTro.Models.ViewModels
{
    public class SideBarSearchModel
    {
        ...
        {
            DateTime date = DateTime.Parse("1/1/0001 12:00:00 AM");
            if (ngayBD == date)
            {
                ngayBD = DateTime.Now;
            }
        ...
        public List<BaiDang> getPosterSearch(int GiaTien,string name, DateTime? ngayBD, DateTime ngayKT, int recSkip, int pageSize)
        {
            var model = _context.baiDangs.Where(res => res.flag == false && res != null).ToList();
            System.Diagnostics.Debug.WriteLine(ngayBD, "Thanglog");
            DateTime date = DateTime.Parse("01/01/0001 12:00:00 AM");
            if (ngayBD == date)
            {
                ngayBD = DateTime.Now;
            }
            if (name != null&& GiaTien==0)
            {
            ...
            else if(name== null && GiaTien>0)
            {
                ...
                .Join(
                    ...
                    tpk => tpk.Id,
                    (tfk, tpk) => new { BaiDang = tfk.BaiDang, PhongTro = tfk.PhongTro, TinhThanh = tpk })
                .Where(res => res.BaiDang.NgayTao < ngayBD
                              && res.PhongTro.Gia == GiaTien
                              && res.BaiDang.flag == false
                              )
                .Skip(recSkip)
                .Take(pageSize)
            ...
            else 
            {
                ...
                .Join(
                    ...
                    (tfk, tpk) => new { BaiDang = tfk.BaiDang, PhongTro = tfk.PhongTro, TinhThanh = tpk })
                .Where(res => res.BaiDang.NgayTao < ngayBD
                              && res.BaiDang.PhongTro.TinhThanh.TenTinh.Contains(name)
                              && res.PhongTro.Gia == GiaTien
                              && res.BaiDang.flag == false
                              )
                .Skip(recSkip)
        ...
        // Tìm kiếm phòng trong booked room
