    ...
    public class PosterModel
    {
        ...
        public async Task<int> CountPosterUser(string userID)
        {
                return await _context.baiDangs
                    .Include(res => res.PhongTro)
                    .Where(res => res.ApplicationUserId == userID && res.flag == false)
                    .CountAsync();
        }

        public async Task<int> CountPosterUser2(string userID, int giatien)
        {
            if (giatien == null)
            {
                return await _context.baiDangs
            ...
            else
            {
                return await _context.baiDangs
                    .Include(res => res.PhongTro)
                    .Where(res => res.ApplicationUserId == userID && res.flag == false && res.PhongTro.Gia == giatien)
                    .CountAsync();
            }
        }
        ...
        public List<BaiDang> getAllPosters_Price(int giatien,string userID, int recSkip, int pageSize)
        {
            var posters = _context.baiDangs
                .Join(
                    _context.phongTros,
                ...
                .Join(
                    ...
                    tpk => tpk.Id,
                    (tfk, tpk) => new { BaiDang = tfk.BaiDang, PhongTro = tfk.PhongTro, TinhThanh = tpk })
                .Where(res => res.BaiDang.ApplicationUserId == userID 
        ...
        // sắp xếp 
        public async Task<List<ApiGetPosters>> getPosterSort(int pageSize, int recsSkip)
        {
