...
namespace QuanLyPhongTro.Models.ViewModels
{
    public class BookedModel
    {
        ...
        // =======================================================================
        // tìm kiếm phòng đã đặt theo địa chỉ 
        public List<ChiTietDatPhong> GetRoomBookedTheoTinhThanh(string userID, int skip, int size,string tinhthanh)
        {
            var result = _roomManagementContext.chiTietDatPhongs
                .Join(
                    _roomManagementContext.phongTros,
                ...
                .Join(
                    ...
                    tt => tt.Id,
                    (ctp, tt) => new { ChiTietDatPhong = ctp.ChiTietDatPhong, PhongTro = ctp.PhongTro, ApplicationUser = ctp.ApplicationUser, TinhThanh = tt })
                .Where(res => res.ChiTietDatPhong.ApplicationUserId == userID&& res.TinhThanh.TenTinh.Contains(tinhthanh) && res.ChiTietDatPhong.flag == false)
                .Skip(skip)
                .Take(size)
                .ToList();
            if (result.Count == 0)
