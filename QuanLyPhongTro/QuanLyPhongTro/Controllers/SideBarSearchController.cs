...
namespace QuanLyPhongTro.Controllers
{
    ...
    public class SideBarSearchController : ComponentsController
    {
        ...
        public async Task <IActionResult> SearchAll(int GiaTien,string name, DateTime ngayBD, DateTime ngayKT, int pg = 1) 
        {
            Authencation();
            ...
            var list = model.getPosterSearch(GiaTien,name, ngayBD, ngayKT, recSkip, pageSize);
            var viewModel = viewModelSide(list);
            this.ViewBag.Pager = pager;
            ViewBag.ValueSearch = name;
            ViewBag.ValueGiaTien = GiaTien;
        ...
        //- tìm kiếm trong quản lý đặt phòng 
        //public async Task<IActionResult> SearchPhongThue(string name, DateTime ngayBD, DateTime ngayKT, int pg = 1)
        //{
        //    Authencation();
